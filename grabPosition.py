...
def measure_slopes(b_matrix, measured_slopes):
    return np.linalg.lstsq(b_matrix, measured_slopes, rcond=None)[0]

def wavefront_reconstruction(reference_positions, delta_x, delta_y, mode_range, measured_slopes):
    b_matrix = form_b_matrix(reference_positions, delta_x, delta_y, mode_range)
    zernike_coefficients = measure_slopes(b_matrix, measured_slopes)
